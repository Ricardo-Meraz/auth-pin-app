...
const ConfigPin = () => {
  ...
  const guardarPin = async (e) => {
    ...
    if (!email) {
      setMensaje("No se encontró el correo del usuario. Vuelve a iniciar sesión.");
      return;
    }

    try {
      const res = await api.post("/auth-pin/configurar-pin", {
        email,
        pin,
      });
      ...
      // Marcamos que el usuario YA tiene PIN
    ...
    <>
      ...
      <div className="container card-pin mt-5">
        ...
        <p className="subtitle">
          Estás configurando el PIN para: <strong>{email || "Sin correo"}</strong>
        </p>
